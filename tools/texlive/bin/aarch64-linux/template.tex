...
\institute[UC]{FH Campus Wien \\ \smallskip \textit{sm@gmail.com}} % Your institution, the optional parameter can be used for the institution shorthand and will appear on the bottom of every slide after author names, while the required parameter is used on the title slide and can include your email address or additional information on separate lines
...
%	Die Ableitung einer Umkehrfunktion $f^{-1}(y)=x(y)$ erhalten wir ganz leicht \"uber die Leibnitzschreibweise folgenderma\ss en:
%	\begin{equation}
...
\begin{frame}
	\frametitle{Differenzialrechnung}
	Die Ableitung einer Umkehrfunktion $f^{-1}(y)=x(y)$ erhalten wir ganz leicht über die Leibnitzschreibweise folgendermaßen:
	\begin{equation}
		\frac{dx}{dy}=\frac{1}{\frac{dy}{dx}}
	\end{equation}
	\begin{exampleblock}{Ein einfaches Beispiel, das häufig bei Differentialgleichungen auftritt, bei denen das Ergebnis dieser Rechnung integriert wird, ist die Ableitung von $\ln(x)$ nach x.}
	\begin{equation}
		\frac{d\ln(y)}{dy}=\frac{1}{\frac{de^x}{dx}}=\frac{1}{e^x}=\frac{1}{y}
	\end{equation}
	\end{exampleblock}
	\begin{alertblock}{Damit ist das häufig bei Differentialgleichungen auftretende Integral über $\frac{1}{x}$:}
	\begin{equation}
...
\begin{frame}
	...
	\begin{exampleblock}{Mit folgendem Trick l\"asst sich ln(x) so integrieren:}
		\begin{equation}
			\int 1\cdot \ln(x)dx = x\cdot \ln(x) + c -\int \frac{x}{x}dx = x\cdot \ln(x) + x + c = x(\ln(x) + 1) + c
		\end{equation}
	\end{exampleblock}
\end{frame}
\begin{frame}
	...
	\begin{exampleblock}{So l\"asst sich auch das Integral \"uber $\sin(x)\cdot \cos(x)$ durchf\"uhren.}
		\begin{equation}
			\int \sin(x)\cdot \cos(x)dx = (\sin(x))^2+c-\int \cos(x)\cdot \sin(x)dx\bigg| +\int \cos(x)\cdot \sin(x)dx
		\end{equation}
		\begin{equation}
...
\begin{frame}
	\begin{definition}
		Es gibt noch die Unterscheidung zwischen \alert{homogen} und \alert{inhomogen} Dgln. Bei inhomogenen Dgln. tritt noch ein Ausdruck auf, der von der Variablen abh\"angt, von der die gesuchte Funktion abh\"angt, beziehungsweise einer Konstanten.
	\end{definition}
	\begin{exampleblock}{Beispiel}
